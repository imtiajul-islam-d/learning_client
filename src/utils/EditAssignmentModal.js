...
export default function EditAssignmentModal({ open, control, id }) {
  //  get editable assignment
  const { data: assignment } = useGetAssignmentQuery(id);
  //  get editable assignment end

  const [
    editAssignment,
    { isLoading, isSuccess, isError: isEditingError, error: editingError },
  ] = useEditAssignmentMutation();
  ...
  useEffect(() => {
    if (isEditingError) {
      setError(editingError?.error);
    }
  }, [editingError, isEditingError]);
  return (
    open && (
      <>
        <div
            ...
            <div className="rounded-md shadow-sm -space-y-px">
              ...
              <div>
                ...
                >
                  ...
                  {videos?.map((video) => (
                    <option
                      selected={form?.video_title === video?.title}
                      key={video?._id}
